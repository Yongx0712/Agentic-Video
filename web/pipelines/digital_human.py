...
async def _finalize_digital_human_post(
    final_video_path: str,
    task_dir: str,
    narration: str,
    video_params: dict,
) -> str:
    """本地成片增强：字幕 / 样式 / 画中画 / 关键字高亮 / 音效（失败则退回原片）。"""
    if not video_params.get("post_enhance", True):
        return final_video_path
    pip_disk = None
    up = video_params.get("pip_upload")
    if up is not None:
        ext = os.path.splitext(up.name)[1] or ".mp4"
        pip_disk = os.path.join(task_dir, "pip_layer" + ext)
        with open(pip_disk, "wb") as f:
            f.write(up.getbuffer())
    from pixelle_video.postprocess.digital_human_post import enhance_digital_human_video_safe
    ...
    return await asyncio.to_thread(
        ...
        narration_clean,
        pip_disk,
        subtitle_font=video_params.get("post_subtitle_font"),
        subtitle_font_size=video_params.get("post_subtitle_font_size"),
        subtitle_color_hex=video_params.get("post_subtitle_color_hex"),
    )
...
class DigitalHumanPipelineUI(PipelineUI):
    ...
    def _render_output_preview(self, pixelle_video: Any, video_params: dict):
        ...
        with st.container(border=True):
            ...
            with st.expander(tr("digital_human.post.section"), expanded=False):
                video_params["post_enhance"] = st.checkbox(
                    tr("digital_human.post.enable"),
                    value=True,
                    key="dh_post_enhance",
                )
                video_params["pip_upload"] = st.file_uploader(
                    tr("digital_human.post.pip_upload"),
                    type=["mp4", "mov", "webm", "mkv"],
                    key="dh_pip_upload",
                )
                video_params.update(render_subtitle_post_controls("dh_post"))
                st.caption(tr("digital_human.post.hint"))
                ...
                try:
                    ...
                    async def generate_digital_human_video():
                        ...
                        if mode == "customize":
                            ...
                            status_text.text(tr("digital_human.post.running"))
                            progress_bar.progress(92)
                            final_video_path = await _finalize_digital_human_post(
                                final_video_path, task_dir, generated_text, video_params
                            )
