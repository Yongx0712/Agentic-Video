...
class PixelleVideoCore:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self._comfykit_loop_id: Optional[int] = None
    ...
    async def _get_or_create_comfykit(self) -> ComfyKit:
        ...
        config_changed = self._comfykit_config_hash != current_hash
        loop_changed = (
            self._comfykit is not None
            and self._comfykit_loop_id is not None
            and self._comfykit_loop_id != loop_id
        )

        if self._comfykit is None or config_changed or loop_changed:
            if self._comfykit is not None:
                if config_changed:
                    logger.info("🔄 ComfyUI configuration changed, recreating ComfyKit instance...")
                elif loop_changed:
                    logger.info("🔄 Event loop changed, recreating ComfyKit instance...")
                try:
                    await self._comfykit.close()
                except Exception as e:
                    logger.warning(f"Failed to close old ComfyKit instance: {e}")
                self._comfykit = None
                self._comfykit_loop_id = None
            ...
            self._comfykit_loop_id = loop_id
